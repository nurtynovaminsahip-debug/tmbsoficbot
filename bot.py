...
async def post_to_channel(bot, text, file_id='', file_type='', ann_id=0):
    ...
    ch = config.CHANNEL_USERNAME
    logger.info(f"post_to_channel: chat_id={ch!r} (type={type(ch).__name__}) file_type={file_type!r}")
    try:
        if file_type == 'photo' and file_id:
            msg = await bot.send_photo(ch, file_id, caption=text, reply_markup=kb)
        elif file_type == 'video' and file_id:
            msg = await bot.send_video(ch, file_id, caption=text, reply_markup=kb)
        else:
            msg = await bot.send_message(ch, text, reply_markup=kb)
        logger.info(f"post_to_channel: success, message_id={msg.message_id}")
        return msg.message_id
    except Exception as e:
        logger.error(f"Error posting to channel (chat_id={ch!r}): {e}")
        return 0
...
def owner_panel_kb():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔔Тест канала", callback_data="op:test_channel"),
...
async def handle_owner_panel_action(q, user, ctx, action):
    ...
    if cmd == "test_channel":
        ch = config.CHANNEL_USERNAME
        await q.answer("🔔 Отправляю тест...", show_alert=False)
        try:
            msg = await ctx.bot.send_message(ch, "🔔 Тест публикации от владельца. Всё работает!")
            await ctx.bot.delete_message(ch, msg.message_id)
            result_text = (
                f"✅ Успех!\n\n"
                f"Канал: <code>{ch}</code> (тип: <b>{type(ch).__name__}</b>)\n"
                f"Сообщение отправлено и удалено."
            )
        except Exception as e:
            result_text = (
                f"❌ Ошибка публикации\n\n"
                ...
                f"• У бота есть право «Публикация сообщений»?\n"
                f"• ID канала верный? Forwarding msg из канала в @userinfobot покажет точный ID"
            )
        await safe_send(ctx.bot, user.id, result_text, parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("🔙Назад", callback_data="con:owner_panel")
            ]]))
        return
...
def main():
    ...
    logger.info(f"Config: CHANNEL_USERNAME={config.CHANNEL_USERNAME!r} ADMIN_GROUP_ID={config.ADMIN_GROUP_ID} OWNER_ID={config.OWNER_ID}")
